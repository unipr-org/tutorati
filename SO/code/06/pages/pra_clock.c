#include "settings.h"
#include "pra.h"
#include <stdio.h>
#include <stdbool.h>

static uint32_t num_page_faults = 0U;

struct page_table_entry {
  uint32_t frame;
  bool valid;
  bool used;
};

static struct page_table_entry page_table[NUM_PAGES] = { 0 };

static uint32_t time = 0U;

static uint32_t next_free_frame = 0U;

void pra_initialize(uint32_t num_refs) {
  (void) num_refs;
}

static uint32_t next_candidate_victim = 0U;

uint32_t select_frame(uint32_t page_in_need) {
  if (next_free_frame < NUM_FRAMES) {
    return next_free_frame++;
  }

  // All frames have been initially assigned: need to select a victim.
  while (true) {
    if (page_table[next_candidate_victim].valid) {
      if (page_table[next_candidate_victim].used) {
        page_table[next_candidate_victim].used = false;
      }
      else {
        break;
      }
    }
    next_candidate_victim = (next_candidate_victim + 1U) % NUM_PAGES;
  }

#if VERBOSE
  printf("At time %u, assign frame %u from page %u to page %u\n",
         time,
         page_table[next_candidate_victim].frame,
         next_candidate_victim,
         page_in_need);
#else
  (void) page_in_need;
#endif

  // lowest_time_page is the index of the victim page.
  page_table[next_candidate_victim].valid = false;
  return page_table[next_candidate_victim].frame;
}

uint32_t pra_reference(uint32_t logical_page) {
  if ((time % NUM_REFERENCES_IN_QUANTUM) == 0U) {
    for (uint32_t p = 0U; p < NUM_PAGES; ++p) {
      page_table[p].used = false;
    }
  }

  uint32_t physical_frame = 0U;

  if (page_table[logical_page].valid) {
    physical_frame = page_table[logical_page].frame;
  }
  else {
    // Page fault.
    ++num_page_faults;
    physical_frame = select_frame(logical_page);
    page_table[logical_page].frame = physical_frame;
    page_table[logical_page].valid = true;
  }
  page_table[logical_page].used = true;
  ++time;
  return physical_frame;
}

uint32_t pra_page_faults(void) {
  return num_page_faults;
}
