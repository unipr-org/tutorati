...
uint32_t select_frame(uint32_t page_in_need) {
  ...
  uint32_t lowest_use_count_page = 0U;
  uint32_t lowest_use_count = UINT32_MAX;
  for (uint32_t page = 0; page < NUM_PAGES; ++page) {
    if (page_table[page].valid && page_table[page].use_count < lowest_use_count) {
      lowest_use_count = page_table[page].use_count;
      lowest_use_count_page = page;
    }
  }
  ...
  printf("At time %u, assign frame %u from page %u (used %u times) to page %u\n",
         time,
         page_table[lowest_use_count_page].frame,
         lowest_use_count_page,
         lowest_use_count,
         page_in_need);
#else
  (void) page_in_need;
  ...
  // lowest_use_count_page is the index of the victim page.
  page_table[lowest_use_count_page].valid = false;
  return page_table[lowest_use_count_page].frame;
}
...
uint32_t pra_reference(uint32_t logical_page) {
  ...
  if (page_table[logical_page].valid) {
    ...
    // Inizializzo il contatore di utilizzo a 1
