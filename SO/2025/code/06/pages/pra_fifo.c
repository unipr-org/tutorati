...
uint32_t select_frame(uint32_t page_in_need) {
  // If there are free frames, assign one.
  ...
  for (uint32_t page = 0; page < NUM_PAGES; ++page) {
    if (page_table[page].valid && page_table[page].entry_time < lowest_time) {
      lowest_time = page_table[page].entry_time;
      lowest_time_page = page;
    }
  }

#if VERBOSE
  printf("At time %u, assign frame %u from page %u (entered at time %u) to page %u\n",
         time,
...
uint32_t pra_reference(uint32_t logical_page) {
  ...
  if (page_table[logical_page].valid) {
    ...
    page_table[logical_page].entry_time = time;
  }
  ++time;
